        ...
        void OnEnableAllRCBsClick(object sender, EventArgs e)
            {
                ...
                var ld = tsi?.Tag as NodeLD;
                if (ld == null) { MessageBox.Show("请从 IED 视图的 LD 上下文菜单触发。"); return; }

                var iecsld = ld.GetIecs();
                if (iecsld == null) { MessageBox.Show("未找到 IED 基础结构或未连接。"); return; }

                int enabledReportsCnt = 0;
                var sb = new System.Text.StringBuilder();

                var children = ld.GetChildNodes();
                if (children == null || children.Length == 0)
                {
                    MessageBox.Show("该 LD 下没有子节点。");
                    return;
                }
                ...
                {
                    var vl = nd as NodeVL;
                    if (vl == null) continue;         // 只处理 VL
                    if (vl.Activated) continue;       // 已激活就跳过

                    var iecs = vl.GetIecs();
                    if (iecs == null) continue;

                    NodeData pickedRcb = null;        // 选中的 RCB（URCB/BRCB 皆可）
                    NodeBase cursor = null;

                    // 按可见字符串（IEC 地址）查找，直到命中 *rcb* 上层节点
                    do
                    {
                        cursor = iecs.DataModel?.ied?.FindNodeByValue(scsm_MMS_TypeEnum.visible_string, vl.IecAddress, ref cursor);
                        if (cursor == null) break;

                        var candidate = cursor as NodeData;
                        var parentData = candidate?.Parent as NodeData;
                        bool parentIsRcb = parentData?.Name?.IndexOf("rcb", StringComparison.OrdinalIgnoreCase) >= 0;
                        if (!parentIsRcb) continue;

                        // 如果有 Resv，优先选择 Resv == false（未被保留）
                        bool ok = true;
                        var resv = parentData.FindChildNode("Resv") as NodeData;
                        if (resv?.DataValue is bool resvVal) ok = (resvVal == false);

                        // 如果有 RptEna，也尽量选择未启用的
                        var rptEna = parentData.FindChildNode("RptEna") as NodeData;
                        if (ok && rptEna?.DataValue is bool rptVal) ok = (rptVal == false);

                        if (ok) { pickedRcb = candidate; break; }

                    } while (cursor != null);

                    if (pickedRcb == null) continue; // 找不到可用 RCB 就跳过

                    var d = pickedRcb.Parent as NodeData;  // 真正的 RCB 容器节点
                    if (d == null) continue;

                    var writes = new List<NodeData>();

                    // 小工具：按模板节点构造写入项
                    NodeData Add(NodeData tpl, object value, int? param = null)
                    {
                        if (tpl == null) return null;
                        var n = new NodeData(tpl.Name) { DataType = tpl.DataType, DataValue = value };
                        if (param.HasValue) n.DataParam = param.Value;
                        writes.Add(n);
                        return n;
                    }

                    // Resv -> true（有则写）
                    Add(d.FindChildNode("Resv") as NodeData, true);

                    // DatSet -> 沿用现值（有则写）
                    var ds = d.FindChildNode("DatSet") as NodeData;
                    if (ds != null) Add(ds, ds.DataValue);

                    // OptFlds -> 0x7c00（与原项目一致），DataParam=6
                    var of = d.FindChildNode("OptFlds") as NodeData;
                    if (of != null) Add(of, new byte[] { 0x7c, 0x00 }, 6);

                    // TrgOps -> 0x74（与原项目一致），DataParam=2
                    var to = d.FindChildNode("TrgOps") as NodeData;
                    if (to != null) Add(to, new byte[] { 0x74 }, 2);

                    // RptEna -> true（有则写）
                    Add(d.FindChildNode("RptEna") as NodeData, true);

                    // GI -> true（触发一次总召；有则写）
                    Add(d.FindChildNode("GI") as NodeData, true);

                    if (writes.Count == 0) continue;

                    try
                    {
                        iecs.Send(writes.ToArray(), d.CommAddress, ActionRequested.Write);
                        vl.Activated = true;
                        enabledReportsCnt++;
                        sb.AppendLine(d.Name ?? pickedRcb.Parent?.Name ?? "RCB");
                    }
                    catch (Exception ex)
                    {
                        // 可按需弹框或写日志
                         MessageBox.Show($"启用 {d.Name} 失败: {ex.Message}");
                    }
                }

                MessageBox.Show(
                    $"Enabled {enabledReportsCnt} {(enabledReportsCnt == 1 ? "report" : "reports")}:\r\n\r\n{sb}\r\n",
                    "Enable All RCBs"
                );
            }
